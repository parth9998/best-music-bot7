...
const { canModifyQueue } = require("../util/djvenom");
...
module.exports = {
  async play(song, message) {
    ...
    try {
      var playingMessage = await queue.textChannel.send(`Started playing :**${song.title}** Requested by ${message.author} `);
    ...
    collector.on("collect", (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        case "🔉":
          ...
          queue.textChannel
            .send(`${user} 🔉 decreased the volume to ${queue.volume}%`)
            .catch(console.error);
          break;
        ...
        case "🔊":
          ...
          queue.connection.dispatcher.setVolumeLogarithmic(queue.volume / 100);
          queue.textChannel
            .send(`${user} 🔊 increased the volume to ${queue.volume}%`)
            .catch(console.error);
          break;
